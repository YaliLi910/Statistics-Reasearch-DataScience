...
data<-data%>%filter(SPECIES=='Atlantic Cod' | SPECIES == 'Atlantic Herring')

data<-data%>%filter(EPU=="GB")
...
data_herr<-data%>%select("ID","SPECIES","biomass")
colnames(data_herr)[colnames(data_herr)=="biomass"] <- "biomass_herring"
data_join<-left_join(data_cod,data_herr,by="ID")

data_clean<-data_join%>%drop_na()
str(data_clean)
#data_clean<-data_clean%>%filter(biomass < 11 & biomass_herring < 4)
data_final<-data_clean%>%select(biomass, biomass_herring, ID, SURFTEMP, BOTTEMP, DEPTH)
setnames(data_final, old=c("biomass","biomass_herring", "SURFTEMP", "BOTTEMP","DEPTH"), 
         new=c("mass_c", "mass_h", "s_temp","b_temp","depth"))
summary(data_final)
...
fit <- lm(mass_c ~ s_temp + b_temp + depth + 
             mass_h,data =data_final)
summary(fit)
#fit model between vairables 
fit1 <- lm(mass_c ~ b_temp+ depth+ 
             mass_h,data =data_final)
summary(fit1)
...
#==============================================================
fit1.stres <- rstandard(fit1)
plot(fit1$fitted.values, fit1.stres, pch=16, 
     main="Standardized Residual Plot", xlab="Fitted Cod biomass", 
     ylab = "Standardized Residuals")
abline(0, 0, lty=2, col="red")

h <- hist(fit1.stres,xlim = c(-6,6))
x <- fit1.stres
...
qqnorm(fit1.stres, main = "Normal Probability Plot",  xlab = "Normal Scores",  ylab = "Standardized Residuals", xlim = c(-2, 2), ylim = c(-2,2))
qqline(fit1.stres, col = "red")

shapiro.test(fit1.stres)
#++========================Try Panel Method======================
...
data123<-data_final%>%select(YEAR,biomass)
data456<-data_final%>%select(YEAR,biomass_herring)
ggplot(data123, aes(x=YEAR, y=biomass)) + geom_point()
data789<-data_final%>%select(biomass,biomass_herring)
